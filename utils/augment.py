...
class DiffAug():
        ...
        if strategy == '' or strategy.lower() == 'none':
            ...
            for aug in strategy.lower().split('_'):
                if aug == 'flip' and single == False:
                    ...
                    self.cutout = True
                elif aug == 'stain' and single == False:
                    T = transforms.Compose([
                        transforms.ToTensor(),
                        transforms.Lambda(lambda x: x * 255)
                    ])
                    he_target = cv2.cvtColor(cv2.imread('dataset/target.jpg'), cv2.COLOR_BGR2RGB)
                    self.normalizer = torchstain.normalizers.MacenkoNormalizer(backend='torch')
                    self.normalizer.fit(T(he_target).cuda())
                    self.stain = True
                else:
    ...
    def stain_fn(self, x, batch=True):
        for i in range(x.shape[0]):
            # norm = Hed_Aug(x[i].cpu())
            # x[i] = torch.from_numpy(norm).cuda()
            norm,_,_ = self.normalizer.normalize(x[i], stains=False)
            norm = torch.permute(norm, (2, 0, 1))
            x[i].data = norm.data
