...
class Meta(nn.Module):
    ...
    def summary_frmil(self, loader, args=None, set='val', thrs=0.5):
        ...
        with torch.no_grad():
            for batch_idx, (data_ft, proto_label, label) in enumerate(tqdm_gen, 1):
                data_ft, proto_label, label = data_ft.to(device), proto_label.to(device), label.to(device)
                prompted_data = self.get_prompted_ft_based_on_stain(data_ft, proto_label, self.prompter_gather)
        ...
        with torch.no_grad():
            for batch_idx, data in tqdm(enumerate(loader)):
                if self.args.use_proto:
                    data_ft, label = data
                    if len(data_ft.size()) > 2:
                        _, bag_size, ft_size = data_ft.size()
                        data_ft, proto_label = data_ft[:, :, :ft_size-1], data_ft[:, :, ft_size-1]
                    else:
                        ft_size = data_ft.size(1)
                        data_ft, proto_label = data_ft[:, :ft_size-1], data_ft[:, ft_size-1]
                    # ft_size = data_ft.size(1)
                    # data_ft, stain_ft = data_ft[:, :ft_size - 1], data_ft[:, ft_size - 1]
                    data_ft, proto_label, label = data_ft.to(device), proto_label.to(device), label.to(device)
                    prompted_data = self.get_prompted_ft_based_on_stain(data_ft, proto_label, self.prompter_gather)
                    del proto_label
                else:
    ...
    def get_prompted_ft_based_on_stain(self, h, h_proto, prompter_gather=None):
        prompted_image = []
        ...
        if len(h.size()) > 2:
            ...
            for i in range(h.size(0)):
                ...
                h_i = h[i]
                h_proto_i = h_proto[i]
                unique_idx = torch.unique(h_proto_i).to(torch.long)
                for i in unique_idx:
                    idx_h = torch.where(h_proto_i == i)[0].to(torch.long)
            ...
            for i in unique_idx:
                ...
                prompted_image.append(
                    prompter_gather[i](h[idx_h])
                )
            prompted_image = torch.cat(prompted_image, dim=0)
        # print('Size check ', h_stain.size(), h.size(), prompted_image.size())
        # with torch.no_grad():
        # if len(h.size()) > 2:
        #     reform = True
        #     b, n, _ = h.size()
        #     h_stain = h_stain.view(-1)
        #     h = h.view(-1, h.size(-1))
        # indices = h_stain
        # unique_idx = torch.unique(indices).to(torch.long)
        ...
        #     idx_h = torch.where(indices == i)[0].to(torch.long)
        #     prompted_image.append(
        #         prompter_gather[idx_h](h[idx_h])
        #     )        # else:
        # #     reform = False
        #     with torch.no_grad():
        #         if len(h_stain.size()) > 2:
        #             reform = True
        #             b,n,_ = h_stain.size()
        #             h_stain = h_stain.view(-1, h_stain.size(-1))
        #             h = h.view(-1, h.size(-1))
        #         rep_batch = h_stain # [N, emd_dim]
        #         rep_batch_sum = (rep_batch**2).sum(dim=-1, keepdims=True) # [N, 1]
        #         prototype_gather_sum = (self.stain_prototype**2).sum(dim=-1, keepdims=True).T # [1, M]            #### TODO ####
        #         print('size check ', rep_batch_sum.size(), prototype_gather_sum.size(), rep_batch.size(), self.stain_prototype.size())
        #
        #         distance_matrix = torch.sqrt(rep_batch_sum + prototype_gather_sum - 2 * torch.mm(rep_batch, self.stain_prototype.T)) # [N, M]
        #         indices = torch.argmin(distance_matrix, dim=-1) # [B]
        #
        #     for idx in range(h_stain.size(0)):
        #         prompted_image.append(
        #             prompter_gather[indices[idx]](h[idx].unsqueeze(0)))
        # prompted_image = torch.cat(prompted_image, dim=0)
        # print('Size check ', h_stain.size(), h.size(), prompted_image.size())
        # if prompted_image.size(0) < h.size(0):
        #     from_to = h.size(0) - prompted_image.size(0)
        #     prompted_image = torch.cat([prompted_image, h[from_to:]], dim=0)
        # prompted_image = prompter_gather[0](h)
        # if reform:
        #     prompted_image = prompted_image.view(b, n, -1)
    ...
    def get_optim(self, train_split):
        if self.num_coarse_stain_classes > 1:
            cluster_counts = torch.zeros(self.num_coarse_stain_classes)
            print('Calculating cluster counts...')
            for i in tqdm(range(len(train_split))):
                ...
                if len(data_ft.size()) > 2:
                    ...
                    ft_size = data_ft.size(1)
                    data_ft, proto_label = data_ft[:, :ft_size - 1], data_ft[:, ft_size - 1]
                unique_label = torch.unique(proto_label)
                for l in unique_label:
                    idx_h = torch.where(proto_label == l)[0].to(torch.long)
                    cluster_counts[int(l)] += len(idx_h)
    ...
    def forward(self, iter, datasets):
        ...
        print('\nInit optimizer ...', end=' ')
        optimizer = self.get_optim(train_split)
        print('Done!')
        ...
        for epoch in range(self.args.max_epochs):
            ...
            for batch_idx, data in enumerate(tqdm_gen):
                ...
                if self.args.use_proto:
                    ...
                    if len(data_ft.size()) > 2:
                        ...
                        data_ft, proto_label = data_ft[:, :ft_size-1], data_ft[:, ft_size-1]
                    data_ft, proto_label, label = data_ft.to(device), proto_label.to(device), label.to(device)
                    # print('Prompting based on stain features...', data_ft.size(), stain_ft.size())
                    prompted_data = self.get_prompted_ft_based_on_stain(data_ft, proto_label, self.prompter_gather)
                    # prompted_data = self.get_prompted_ft(data_ft, self.prompter_gather)
                    # del stain_ft
                else:
                    data_ft, label = data
                    data_ft, label = data_ft.to(device), label.to(device)
                    # prompted_data = self.get_prompted_ft_based_on_stain(data_ft_base, data_ft_stain, self.prompter_gather)
                    # print('Prompting based on stain features...', data_ft.size())
                    prompted_data = self.get_prompted_ft(data_ft, self.prompter_gather)
            ...
            if epoch % 1 == 0:
                test_error, test_auc, test_f1 = self.summary(test_loader,
                                                            ...
                                                            modelname=self.args.mil_method)
                test_acc = 1 - test_error
                log_info = '======> @ Epoch {} Test acc: {:.4f}, ROC AUC: {:.4f}, Test F1: {:.4f}, LR: {:.4f}'.format(epoch,
                                                                                                                test_acc,
                                                                                                                test_auc,
                                                                                                                test_f1,
                ...
                # for i in range(len(self.prompter_gather)):
                #     print(f'Prompt [{i}] changed from {initial_prompt[i].prompt_embeddings[0, 0]} to {self.prompter_gather[i].prompt_embeddings[0, 0]}')
        self.logger('========> [Iter %d] Bestc: %.3f (Acc), %.3f (F1)' % (iter, best_test_acc, best_test_f1))
