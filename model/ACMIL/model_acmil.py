...
from model.ACMIL.network import Classifier_1fc, DimReduction, DimReduction1
from einops import repeat
from .nystrom_attention import NystromAttention
from model.ACMIL.emb_position import *
...
class ACMIL_GA(nn.Module):
    def __init__(self, model_size, n_class, D=128, droprate=0, n_token=1, n_masked_patch=0, mask_drop=0):
        super(ACMIL_GA, self).__init__()
        self.size_dict = {"ultra-tiny":[96,128], "tiny": [192, 128], "ultra_small": [384, 192], 'medium': [512, 512],
                          "small": [1024, 512], "big": [2048, 512]}
        self.dimreduction = DimReduction(self.size_dict[model_size][0], self.size_dict[model_size][1])
        self.attention = Attention_Gated(self.size_dict[model_size][1], D, n_token)
        self.classifier = nn.ModuleList()
        for i in range(n_token):
            self.classifier.append(Classifier_1fc(self.size_dict[model_size][1], n_class, droprate))
        self.n_masked_patch = n_masked_patch
        self.n_token = n_token
        self.Slide_classifier = Classifier_1fc(self.size_dict[model_size][1], n_class, droprate)
        self.mask_drop = mask_drop
