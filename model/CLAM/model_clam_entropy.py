...
class CLAM_SB_entrpy(nn.Module):
    ...
    def inst_eval(self, A, h, classifier):
        ...
        kde = gaussian_kde(attention_scores)
        densities = kde.evaluate(attention_scores)
        local_probabilities = densities / densities.sum()
        local_entropies = -local_probabilities * np.log(local_probabilities)
        local_entropies = np.nan_to_num(local_entropies)
        local_entropies_tensor2 = torch.tensor(local_entropies).to(device)

        top_p_ids2 = torch.topk(A, self.k_sample)[1][-1]
        extracted_entropy_scores = torch.index_select(local_entropies_tensor2, dim=0, index=top_p_ids2)

        top_10_entropy_scores, top_10_indices = torch.topk(extracted_entropy_scores, self.k_sample, largest=False)
        top_p_ids = top_p_ids2[top_10_indices]
        top_p = torch.index_select(h, dim=0, index=top_p_ids)

        top_n_ids2 = torch.topk(-A, self.k_sample, dim=1)[1][-1]
        extracted_entropy_scores = torch.index_select(local_entropies_tensor2, dim=0, index=top_n_ids2)

        top_10_entropy_scores, top_10_indices = torch.topk(extracted_entropy_scores, self.k_sample, largest=False)
        top_n_ids = top_n_ids2[top_10_indices]
